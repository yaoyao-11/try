...
prot=read.csv("./Protr_file/train_feature/ALL_protr_test.CSV",check.names = F)
prot=t(prot)
...
UniRep_RBP=read.csv("./UniRep_file/test_feature/bind_1900_RBPs_train.csv")
UniRep_nonRBP=read.csv("./UniRep_file/test_feature/bind_1900_RBPs_train.csv")
...
seqvec=read.csv("./SeqVec_file/SeqVec_test_features.CSV",check.names = F)
seqvec=t(seqvec)
...
ESM_1b=read.csv("./ESM-1b/Esm1b_test_features.CSV",check.names = F)
ESM_1b=t(ESM_1b)
...
all_test=rbind(RBP_all,nonRBP_all)
all_test=data.frame(all_test,stringsAsFactors = F,check.names = F)
write.csv(all_test,"./bind_all_feature/all_test.csv",quote = F)
